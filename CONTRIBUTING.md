...
## 코드 리뷰
- 지금은 1인 저장소라 다른 사람의 승인을 받을 수 없습니다. 그래서 PR마다 자동 코드 리뷰를 돌려 결과를 PR에 남기고, 지적마다 반영하거나 반영하지 않는 이유를 답글로 단 뒤 병합합니다. #7은 Claude Code `/code-review`의 지적 15건 중 12건을 반영하고 3건은 이유를 남겼습니다. 봇이 올린 의존성 PR도 검토합니다(#6은 액션 10개의 깨지는 변경을 우리 입력값과 대조).

...
``mulmit demo run`` is the one-command path from an empty database to a populated product: it
...
from mulmit.auth.security import hash_password
...
from mulmit.pipeline.ingest import run_ingest
from mulmit.pipeline.link import link_signals
...
from mulmit.sources.registry import FIXTURE_CATALOG, SOURCE_CATALOG, FixtureAdapter, build_adapter

log = get_logger(__name__)

DEMO_PASSWORD = "mulmit-demo-1234"  # noqa: S105 - documented demo credential
ADMIN_PASSWORD = "mulmit-admin-1234"  # noqa: S105
...
async def seed_tenants(session: AsyncSession, runtime: Runtime) -> list[int]:
    ...
    for t in DEMO_TENANTS:
        ...
        if t.plan != "free":
            ...
            from mulmit.billing.service import start_period
...
async def run_demo_pipeline(session: AsyncSession, runtime: Runtime, *, anchor: date) -> DemoReport:
    ...
    if calibration:
        from mulmit.db.models import Opportunity

...
from mulmit.sources import clik, g2b, lofin
from mulmit.sources.base import DocType, FetchWindow, RawRecord, SourceAdapter
from mulmit.sources.http import FatalSourceError, ResilientClient
...
def _world(anchor: str, seed: int, scale: float, scanned_ratio: float) -> Any:
    from mulmit.demo.synth import build_world
...
def build_adapter(source: Source, runtime: Runtime) -> SourceAdapter:
    ...
    if source.adapter == "g2b":
        if not s.data_go_kr_service_key:
            raise FatalSourceError("MULMIT_DATA_GO_KR_SERVICE_KEY is not configured")
    ...
    if source.adapter == "clik":
        if not s.clik_api_key:
            raise FatalSourceError("MULMIT_CLIK_API_KEY is not configured")
    ...
    if source.adapter == "lofin":
        if not s.lofin_api_key:
            raise FatalSourceError("MULMIT_LOFIN_API_KEY is not configured")

...
from mulmit.api.deps import PrincipalDep, RuntimeDep, SessionDep, SettingsDep
from mulmit.api.schemas import LoginIn, MeOut, OrgOut, SignupIn, UserOut
from mulmit.auth.security import COOKIE_NAME, create_token, hash_password, verify_password
from mulmit.billing.service import ensure_subscription, start_period
from mulmit.db.models import AlertChannel, AlertRule, CompanyProfile, Organization, User
...
def _throttle_key(request: Request, email: str) -> str:
    ...
    return f"mulmit:login:{ip}:{email}"
